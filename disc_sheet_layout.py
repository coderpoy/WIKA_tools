...
def visualize_layout(effective_diameter_cm, discs_per_side, sheet_side_cm, diameter_cm, diameter_mm):
    ...
    plt.ylabel("cm")
    plt.grid(True)
    st.pyplot(fig)  # Use Streamlit's way to show matplotlib figures

st.title("Optimal Metal Sheet Layout for Disc Cutouts")

diameter_mm = st.number_input("Enter the diameter of the disc (mm):", min_value=1.0, max_value=1000.0, value=100.0)
quantity = st.number_input("Enter the quantity of discs:", min_value=1, max_value=1000, value=9)

if st.button("Calculate and Visualize"):
    result = calculate_optimal_sheet(diameter_mm, quantity)

    st.subheader("Calculation Summary")
    st.write(f"**Disc Diameter:** {diameter_mm} mm")
    st.write(f"**Quantity Requested:** {quantity}")
    st.write(f"**Clearance Between Discs:** 10 mm")
    st.write(f"**Effective Diameter (including clearance):** {result['effective_diameter_cm']:.2f} cm")
    st.write(f"**Optimal Sheet Side Length:** {result['sheet_side_cm']:.2f} cm")
    st.write(f"**Optimal Sheet Area:** {result['sheet_area_cm2']:.2f} cm²")
    st.write(f"**Discs per Side:** {result['discs_per_side']}")
    st.write(f"**Total Discs Possible:** {result['total_discs_possible']}")
